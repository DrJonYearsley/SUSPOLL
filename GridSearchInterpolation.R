...
h1_minvalue <- hist(i0_values_list, breaks=25, plot=F)
h2_minvalue <- hist(E_values_list, breaks=25, plot=F)
top_minvalue <- max(h1_minvalue$counts, h2_minvalue$counts)
k_minvalue <- kde2d(i0_values_list, E_values_list, n=25)
# margins
oldpar <- par()
par(mar=c(3,3,1,1))
layout(matrix(c(2,0,1,3),2,2,byrow=T),c(3,1), c(1,3))
image(k_minvalue, main='from min value') #plot the image
par(mar=c(0,2,1,0))
...
if(Honeybee==T){
  ...
  write.csv(FlyTemp_values_list,file="Fit_FlyTemp_values_list_HB.csv")
  write.csv(CoolTemp_values_list,file="Fit_CoolTemp_values_list_HB.csv")
}
